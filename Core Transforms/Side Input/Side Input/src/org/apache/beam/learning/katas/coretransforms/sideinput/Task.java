...
import org.apache.beam.vendor.bytebuddy.v1_11_0.net.bytebuddy.matcher.DeclaringMethodMatcher;
...
public class Task {
    ...
    static PCollection<Person> applyTransform(
        ...
        return persons.apply(ParDo.of(
                        new DoFn<Person, Person>() {
                            @ProcessElement
                            public void enrichPerson(@Element Person person, OutputReceiver<Person> emitter, ProcessContext context) {
                                Map<String, String> citiesToCountriesMap = context.sideInput(citiesToCountriesView);
                                String personCity = person.getCity();
                                Person enriched = new Person(
                                        person.getName(),
                                        personCity,
                                        citiesToCountriesMap.get(personCity)
                                );
                                emitter.output(enriched);
                            }
                        }
                )
